<?php
/**
 * Plugin Name: KO Clean Image Filenames
 * Plugin URI:  https://kevinocasio.com/tools/ko-clean-image-filenames/
 ...
 * Text Domain: ko-clean-image-filenames
 */
...
define('KO_CIF_VERSION', '1.0.0');
define('KO_CIF_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('KO_CIF_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ko_cif_brand_url($path = '/tools/') {
 ...
 */
function ko_cif_activate()
{
	if (get_option('ko_cif_enabled') === false) {
		update_option('ko_cif_enabled', '1');
	}
}
register_activation_hook(__FILE__, 'ko_cif_activate');

/**
 * 2. Core Logic: Sanitize Uploaded Filenames
 */
function ko_cif_sanitize_filename($filename)
{
	if ((int) get_option('ko_cif_enabled', '1') !== 1) {
		return $filename;
	}
...
add_filter('sanitize_file_name', 'ko_cif_sanitize_filename', 10);

/**
 * 3. Admin Menu Placement (KO Plugins Suite at Position 2)
 */
function ko_cif_admin_menu()
{
...
add_action('admin_menu', 'ko_cif_admin_menu');

/**
 * 4. AJAX Handler for Instant Dashboard Toggle
 */
function ko_cif_ajax_toggle_status()
{
	check_ajax_referer('ko_cif_toggle_nonce', 'nonce');
	...
	update_option('ko_cif_enabled', $enabled);
...
add_action('wp_ajax_ko_cif_toggle_status', 'ko_cif_ajax_toggle_status');

/**
 * 5. Action Link on Plugins Screen
 */
function ko_cif_settings_action_link($links)
{
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ko_cif_settings_action_link');

/**
 * 6. Conditional Admin Asset Enqueuing (Strict Anti-Bloat)
 */
function ko_cif_enqueue_admin_assets($hook)
{
	if ('toplevel_page_ko-plugins-main' !== $hook) {
		return;
	}
	$ver = file_exists(KO_CIF_PLUGIN_DIR . 'assets/admin.css') ? filemtime(KO_CIF_PLUGIN_DIR . 'assets/admin.css') : KO_CIF_VERSION;
	wp_enqueue_style('ko-cif-admin-css', plugins_url('assets/admin.css', __FILE__), array(), $ver);
}
add_action('admin_enqueue_scripts', 'ko_cif_enqueue_admin_assets');
...
if (!function_exists('ko_plugins_suite_dashboard_html')) {
	function ko_plugins_suite_dashboard_html()
	{
		...
		$hub_url = ko_cif_brand_url('/tools/');
		$author_url = ko_cif_brand_url('/');
		...
		<div class="wrap ko-dash-wrap">
			...
			<div class="ko-dash-grid">
				<?php foreach ($suite_plugins as $plugin):
					...
					$plugin_url = ko_cif_brand_url('/tools/' . $plugin['slug'] . '/');
					?>
